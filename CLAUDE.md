...
| `routes/requirements.py`     | Batch create, list/poll, answer, confirm, edit, restart, delete requirements (create/delete 422 once requirements are locked)                                                                               |
...
| `components/`     | `LoginModal` (full-screen, uncancellable), `RequirementForm` (dynamic rows), `RequirementCard` (per-status), `TestPlanCard` (per-status), `TestPlanEditForm` (steps one-per-line)                                                                         |
