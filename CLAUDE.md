...
**Prerequisites for running the backend:** PostgreSQL running with a `qa_agent` database (`createdb qa_agent`), and `backend/.env` present (copy from `backend/.env.example`). Tables are auto-created on startup by `init_db()`; changes to existing tables ship as idempotent migrations run at startup (see Conventions #11). Requirement analysis and test-plan generation additionally need Redis, a worker (`python -m backend.worker`, second terminal), and an LLM API key (`OPENAI_API_KEY`); without them the rows simply stay `pending`. Tests need none of these (see [Testing](#testing)).
...
| `models/database.py`         | Table models: `Repo`, `Sprint`, `Requirement`, `TestEnvironmentAccess`, `TestPlan`, `TestCase` + status enums (repo → sprints → requirements → one plan each)                                               |
...
| `routes/test_environment.py` | Get / submit+check / answer / confirm the test environment access description (synchronous LLM check via `asyncio.to_thread`)                                                                               |
...
| `services/llm.py`            | OpenAI-SDK client wrapper: `check_clarity`, `revise_requirement`, `check_test_environment`, `revise_test_environment`, `generate_test_plan`, `revise_test_plan` (bounded `read_file` tool loop), `LLMError` |
| `services/reconciler.py`     | `reconcile_once` + `reconciler_loop` — re-enqueues lost jobs, sweeps stale heartbeats + finished sprints (parametrized over `Requirement` and `TestPlan` via `_SWEEP_SPECS`)                                |
| `tasks/`                     | RQ task modules, one file per task type (`analyze_requirement.py`, `generate_test_plan.py`); enqueued by dotted string path                                                                                 |
...
| `router.tsx`      | `createBrowserRouter`: `/` SprintListPage, `/sprints/new` CreateSprintPage, `/sprints/:id` SprintDetailPage, `/sprints/:id/test-environment` TestEnvironmentPage, `/sprints/:id/test-plans` TestPlansPage, `/repos` RepoListPage — all under `RootLayout` |
...
| `components/`     | `LoginModal` (full-screen, uncancellable), `RequirementForm` (dynamic rows), `PrdUploadForm` (PRD upload + replace confirmation), `RequirementCard` (per-status), `TestPlanCard` (per-status), `TestPlanEditForm` (steps one-per-line)                    |
...
- Protected: the `/api/repos*`, `/api/sprints*`, `/api/requirements*`, and test-environment (`/api/sprints/{id}/test-environment`, `/api/test-environment/*`) routers use `dependencies=[Depends(verify_auth)]`. `/api/health` and `/api/auth/*` are open.
