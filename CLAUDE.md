...
- **Windows has no `os.fork()`**: `worker.py` uses `SimpleWorker` when `sys.platform == "win32"` and sets a socket timeout (~5s) on the worker's Redis connection so it never hangs on dead connections.
