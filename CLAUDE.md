...
| `models/database.py`         | Table models: `Repo`, `Sprint`, `Requirement`, `TestEnvironmentAccess` + status enums (repo → sprints → requirements; one test-env row per sprint) |
...
| `services/llm.py`            | OpenAI-SDK client wrapper: `check_clarity`, `revise_requirement`, `check_test_environment`, `revise_test_environment`, `LLMError`                  |
| `services/reconciler.py`     | `reconcile_once` + `reconciler_loop` — re-enqueues lost jobs, sweeps stale heartbeats + finished sprints                                           |
...
| `router.tsx`      | `createBrowserRouter`: `/` SprintListPage, `/sprints/new` CreateSprintPage, `/sprints/:id` SprintDetailPage, `/sprints/:id/test-environment` TestEnvironmentPage, `/repos` RepoListPage — all under `RootLayout` |
