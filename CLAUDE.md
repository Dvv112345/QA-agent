...
| **RQ Worker hangs / won't stop on Windows** | RQ's default `Worker` class uses `os.fork()`, which doesn't exist on Windows. Without a fork, the worker process ignores SIGTERM/SIGINT and must be forcefully killed. Additionally, the default Redis connection has no socket timeout, so the worker can hang indefinitely on a dead connection. RQ's hard-kill `job_timeout` also relies on forking, so on Windows a **cooperative timeout** in the task itself is required — `count_words_task` checks `time.monotonic() - start > timeout` between each file and raises `TimeoutError` to self-terminate gracefully. | Use `SimpleWorker` instead of `Worker` on Windows (`sys.platform == "win32"`). Set `conn.socket_timeout` (5s recommended) so the worker detects dead connections quickly. Pass `JOB_TIMEOUT` as both the cooperative timeout argument to the task and as RQ's `job_timeout` for cross-platform coverage. All three mitigations are handled in `backend/worker.py` and `backend/tasks.py`. |
