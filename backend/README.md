...
| `NONFUNCTIONAL_PLAN_TOOL_ROUNDS`                                                       | `3`                                                      | `read_file` rounds when proposing a run plan, so the model can find which endpoints exist. Deliberately lower than the other tool loops: this one runs synchronously inside a request.                                                                                                                |
...
Each URL records a **four-valued outcome per domain** — `clean`, `violations`, `not_applicable`, `failed_to_run` — because collapsing either of the last two into `clean` states something false. Accessibility applicability is decided by the **media type the server served**, not by how a target was reached: a URL answering `application/json` records `not_applicable` whether it arrived as a discovered endpoint or because the walk navigated to it. That gate cannot be the DOM — Chromium renders JSON through its own viewer, so axe sees real HTML for a body with no user interface at all. An axe run that could not execute, meanwhile, found nothing _because it never ran_. Performance is measured and stored and never judged: it produces no finding, no defect and no ticket, since any threshold would be a verdict on somebody else's capacity planning.
...
Propose which domains apply (with a one-line rationale each), which environment variables hold the application URLs, and which load profiles are worth running. Synchronous, persists nothing. The response also carries **both ceiling tiers** and the list of safe methods, so the setup screen never restates a config value.

The call runs a bounded `read_file` loop (`NONFUNCTIONAL_PLAN_TOOL_ROUNDS`, default 3 — lower than the other two tool loops because this one runs inside a request, so every round is latency a user waits on). A load profile is only worth proposing if it names an endpoint that exists, and the file tree lists paths without routes, so the model reads the repository's routing code to find them. A sprint with no captured file tree degrades to a plain completion.

**The model never sees an environment variable's value.** It is given the variable _names_, split into those holding an `http(s)` URL and the rest, and answers with `base_url_env_var` plus `path` — which key pairs with which endpoint. The server resolves the key and joins the path. That is what makes a proposed profile land on a confirmed origin _by construction_ rather than by the model having guessed the host right; a profile naming a variable it did not nominate as a base URL is dropped. The join is a plain concatenation rather than `urljoin`, which would treat a leading slash as root-relative and discard a base URL's own path prefix.
...
| `per_requirement`                                                | The breakdown, worst first. One row per **covered** requirement, findings or not; archived requirements included and flagged `requirement_deleted`. Each row carries `functional_bug_count` and `nonfunctional_bug_count` beside the `bug_count` total, partitioned by the same `Finding.pool` the headline uses, so the two halves always sum to the total. `bug_count` is what the worst-first sort keys on. |
