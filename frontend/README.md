...
| `/sprints/:id/test-runs`                       | `TestRunsPage`               | QA metrics panel over three lists: exploratory sessions, nonfunctional checks and scripted test runs; start any of them. Also holds the issue-tracker panel — connect, change, or disconnect Jira / GitHub Issues for the sprint                                                                                                                                                                                                                  |
| `/sprints/:id/cicd`                            | `CicdPage`                   | Export verified test scripts to the repository's own CI: connect GitHub Actions or Jenkins, pick which cases to ship, and read the export history. Reached from the test-runs page — a side door off the last stage, deliberately not a stage in `stages.ts`                                                                                                                                                                                      |
...
| `/sprints/:id/nonfunctional-runs/:runId`       | `NonfunctionalRunDetailPage` | One requirement's nonfunctional run: findings with their rule and URL, a per-URL panel of four-valued domain outcomes and measurements, and a per-load-profile panel showing what was actually sent; restart or retry the summary; file or retry bug findings. Performance's `clean` reads as **Measured**, in a neutral chip rather than the green one — it is measured and never judged, so "No violations" would report a threshold nobody set |
...
`FindingCard`, `ExportSummary`, `OutdatedBadge` and `RestartControl` are source-agnostic: scripted, exploratory and nonfunctional results render through the same components.
...
`SprintMetricsPanel` sits above all three run lists on the test-runs page, fed by `GET /api/sprints/:id/qa-metrics` and refreshed on the same poll as the lists. Tiles for test cases run, exploratory sessions, nonfunctional URLs examined (shown only once a nonfunctional run has been counted), distinct bugs, and defect density — plus a per-requirement breakdown ordered worst-first, with archived requirements marked `(deleted)`. The breakdown carries **two** bug columns, functional and nonfunctional, shown on every sprint even when one is all zeros — the two pools never merge into one defect count anywhere else in the app, and a column that appears and disappears between sprints shifts every figure beside it. Each "tests run" figure keeps its own unit and none are ever summed: a 25-action browser session, a 3-step script and a URL examined are not the same thing. Only the bug figure is a distinct-defect count; the issue count beside it is raw, because an issue reports obstructed testing rather than a defect. The bug headline covers **both** defect pools, while both densities divide the functional half — a nonfunctional run finds violations per page, so putting them over test cases would move a number whose denominator never saw them (see `backend/README.md`).
...
Every run shows its id — `Run #14` in the list rows and in the detail headers. That is the number a filed ticket quotes ("Scripted run 14", "Exploratory run 10", "Nonfunctional run 6"), and it is otherwise only in the URL, so a reader holding the ticket has nothing to match against. It is a global identifier rather than a per-sprint count, which is why it reads `#14` rather than "the 14th run".
