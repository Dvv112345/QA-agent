...
`SprintMetricsPanel` sits above all three run lists on the test-runs page, fed by `GET /api/sprints/:id/qa-metrics` and refreshed on the same poll as the lists. Tiles for test cases run, exploratory sessions, nonfunctional URLs examined (shown only once a nonfunctional run has been counted), distinct bugs, and defect density — plus a per-requirement breakdown ordered worst-first, with archived requirements marked `(deleted)`. Each "tests run" figure keeps its own unit and none are ever summed: a 25-action browser session, a 3-step script and a URL examined are not the same thing. Only the bug figure is a distinct-defect count; the issue count beside it is raw, because an issue reports obstructed testing rather than a defect. The bug headline covers **both** defect pools, while both densities divide the functional half — a nonfunctional run finds violations per page, so putting them over test cases would move a number whose denominator never saw them (see `backend/README.md`).
