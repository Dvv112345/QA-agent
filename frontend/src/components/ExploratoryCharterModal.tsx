...
export default function ExploratoryCharterModal({ sprintId, plans, tracker, onClose }: Props) {
  ...
  return (
    <ModalShell title="Start exploratory testing" busy={busy} wide onClose={onClose}>
      {plans.length === 0 ? (
        ...
        <>
          ...
          <button type="button" className="btn btn-secondary" onClick={addCharter} disabled={busy}>
            Add charter
          </button>
        </>
      )}
